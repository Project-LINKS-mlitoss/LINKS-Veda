...
export async function action({ request }: ActionFunctionArgs) {
	...
	try {
		...
		logger.info({
			message: "API spatial aggregate callback",
			data: payload,
		});
		ticketId = payload.ticketId || "";

		config = await prisma.spatialAggregateContentConfigs.findFirst({
		...
		await updateSpatialAggregateContentConfigStatus(
			ticketId,
		...
		console.log("API cross join callback error", error);
		logger.error({
...
async function updateSpatialAggregateContentConfigStatus(
	ticketId: string,
	...
	try {
		...
		await prisma.spatialAggregateContentConfigs.updateMany({
			where: { ticketId: ticketId },
			data: updateData,
		});
	} catch (e) {
		logger.error({
			message: "API spatial aggregate callback: update callback status failed",
			err: e,
		});
	}
}
...
function logResponseError(payload: any) {
	logger.error({
		message: "[RESPONSE] API spatial aggregate callback",
		response: payload,
	});
}
