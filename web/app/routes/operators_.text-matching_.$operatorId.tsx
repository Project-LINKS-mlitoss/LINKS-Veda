...
import {
	generateTextMatchingAction,
	renameContentAction,
} from "~/actions/OperatorAction";
import { saveTemplateTextMatching } from "~/actions/TemplateAction";
import {
	CONTENT_CALLBACK_API_STATUSES_NO_REFETCH,
	OPERATOR_FETCH_TIMEOUT,
} from "~/commons/core.const";
import jp from "~/commons/locales/jp";
import TextMatching from "~/components/pages/Operators/TextMatching";
import { useFetcherWithReset } from "~/hooks/useFetcherWithReset";
import { operatorTextMatchingDetailLoader } from "~/loaders/OperatorLoader";
import {
...
export { operatorTextMatchingDetailLoader as loader };
...
export async function action(actionFunctionArgs: ActionFunctionArgs) {
	...
	switch (actionType) {
		case ACTION_TYPES_OPERATOR.GENERATE: {
			return generateTextMatchingAction({
				...actionFunctionArgs,
				request: newRequest,
			});
		}
		case ACTION_TYPES_OPERATOR.SAVE: {
			return saveTemplateTextMatching({
				...actionFunctionArgs,
				request: newRequest,
			});
		}
...
export default function TextMatchingDetail() {
	const fetcher =
		useFetcherWithReset<ApiResponse<TextMatchingContentConfigs>>();
	const initialResult =
		useLoaderData<ApiResponse<TextMatchingContentConfigs>>();
	const [result, setResult] =
		useState<ApiResponse<TextMatchingContentConfigs> | null>(null);
	...
	useEffect(() => {
		...
		timeoutId = setTimeout(() => {
			fetcher.load(
				`${routes.operator}/${result.data.id}?operatorType=${OPERATOR_TYPE.TEXT_MATCHING}`,
			);
		}, OPERATOR_FETCH_TIMEOUT);
