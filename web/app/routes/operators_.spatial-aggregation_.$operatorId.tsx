...
import { operatorSpatialAggregationDetailLoader } from "~/loaders/OperatorLoader";
import {
...
export { operatorSpatialAggregationDetailLoader as loader };
...
export default function SpatialAggregationDetail() {
	const fetcher =
		useFetcherWithReset<ApiResponse<SpatialAggregationContentConfigs>>();
	const initialResult =
		useLoaderData<ApiResponse<SpatialAggregationContentConfigs>>();
	const [result, setResult] =
		useState<ApiResponse<SpatialAggregationContentConfigs> | null>(null);
	...
	useEffect(() => {
		...
		timeoutId = setTimeout(() => {
			fetcher.load(
				`${routes.operator}/${result.data.id}?operatorType=${OPERATOR_TYPE.SPATIAL_AGGREGATE}`,
			);
		}, OPERATOR_FETCH_TIMEOUT);
