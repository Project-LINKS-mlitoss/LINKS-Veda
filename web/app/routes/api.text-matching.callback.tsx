...
export async function action({ request }: ActionFunctionArgs) {
	...
	try {
		...
		logger.info({
			message: "API text matching callback",
			data: payload,
		});
		ticketId = payload.ticketId || "";
		config = await prisma.textMatchingContentConfigs.findFirst({
		...
		if (
			!payload?.schema?.properties?.matched_data ||
			!payload?.data?.matched_data
		) {
			await updatePreprocessContentConfigStatus(
				ticketId,
		...
		switch (outputType) {
			case OUTPUT_TYPE.JSON: {
				const { newData } = formatJSONData(payload.data.matched_data);
				fileData = JSON.stringify(newData);
				break;
			}
			case OUTPUT_TYPE.GEOJSON: {
				const { newData } = formatGEOJSONData(payload.data.matched_data);
		...
		logger.error({
			message: "API text matching callback failed",
			err: error,
		});
		console.log("API text matching error", error);
		await updatePreprocessContentConfigStatus(
			ticketId,
			CONTENT_CALLBACK_API_STATUS.FAILED,
		);
...
async function updatePreprocessContentConfigStatus(
	...
	try {
		await prisma.textMatchingContentConfigs.updateMany({
			where: { ticketId: ticketId },
			data: { status },
		});
	} catch (e) {
		logger.error({
			message: "API text matching callback: update callback status failed",
			err: e,
		});
	}
}
