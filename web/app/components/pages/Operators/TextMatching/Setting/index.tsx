...
const Setting: React.FC<Props> = (props) => {
	// Props
	const { contentIdLeft, data, workflowDetail, optionColumnsLeft } = props;

	// Remix
	const navigate = useNavigate();
	const submit = useSubmit();
	const actionData = useActionData<ApiResponse<TextMatchingContentConfigs>>();
	...
	>();
	const [contentIdRight, setContentIdRight] = useState<string>("");
	const [leftField, setLeftField] = useState("");
	const [settingTextMatching, setSettingTextMatching] =
		useState<SettingTextMatching>();
	...
	const handleAddContent = () => {
		let newContent: ContentTextMatching;
	...
	const handleAddCol = (contentIndex: number) => {
		setContents((prevContents) => {
			...
			contentToUpdate.column.push({ key: "", value: "" });
	...
	const handleAcceptedTemplate = () => {
		if (tempTemplate) {
			...
			if (workflowDetail && tempChooseWorkflowT?.workflowDetails) {
				...
				navigate(
					`${
						operatorTypeToUrlMap[OPERATOR_TYPE.TEXT_MATCHING]
	...
	useEffect(() => {
		if (contents.length === 1) {
			...
			// Over Handle get Option Columns Right

			const where = [
				{
					leftField: leftField,
					rightField: column[0]?.value,
				},
			];

			const keepRightFields = column
				?.slice(1, column?.length)
				.map((col) => col?.value)
				.filter((value) => value !== "");

			if (keepRightFields.length > 0) {
				setSettingTextMatching({
					where: where,
					keepRightFields,
				});
			} else {
	...
	const handleGenerate = () => {
		...
		formData.set("contentIdLeft", contentIdLeft ?? "");
		formData.set("contentIdRight", contentIdRight ?? "");
		formData.set(
			"settingTextMatching",
			JSON.stringify(settingTextMatching ?? {}),
		);
	...
	useEffect(() => {
		...
		const handleGenerateAction = () => {
			if (actionData?.status === false) {
				...
				navigate(`${routes.operatorTextMatching}/${actionData?.data?.id}`);
		...
		<SettingOperatorS>
						...
						<div className="step">
							...
							<div className="button-template">
								...
								<Button
									...
									disabled={!contents.length || !leftField}
										...
										header={
											<>
												...
												<div className="header-panel-key-value">
													...
													{optionColumnsLeft ? (
														<Select
															onChange={(value) => setLeftField(value)}
			...
			<div className="button-bottom">
				<Button
					type="primary"
					disabled={
						startedGenerate || !contents.length || !contentIdLeft || !leftField
					}
			...
			<ModalSelectTemplate
				...
				operatorType={
					workflowDetail ? OPERATOR_TYPE.WORK_FLOW : OPERATOR_TYPE.TEXT_MATCHING
				}
