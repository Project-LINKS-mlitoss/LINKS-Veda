...
import type { CrossTabContentConfigs } from "~/models/operators";
...
const InputOperator: React.FC<Props> = (props) => {
	// props
	const { setContentId, data, onClickShrinkOutlined, setOptionColumns } = props;
	...
	useEffect(() => {
		if (data?.inputContentId || contentInputId) {
			fetchContentDetail.load(
				`${routes.content}/${data?.inputContentId || contentInputId}`,
			);
		}
	}, [data]);
	...
	useEffect(() => {
		if (selectedContent) {
			...
			setOptionColumns(
				selectedContent?.schema?.fields // No change schema to content because this is data from CMS
