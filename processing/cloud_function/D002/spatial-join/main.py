...
SPATIAL_JOIN_JOB_NAME = os.getenv('SPATIAL_JOIN_JOB_NAME')


class RequestBody(BaseModel):
    inputLeft: str
    inputRight: str
    op: Literal["intersects", "nearest"]
...
def spatial_join(request):
    try:
        ...
        print('[SPATIAL JOIN][INCOMING REQUEST]', req)
...
def trigger_job(data):
    print("[SPATIAL JOIN] calling trigger job")
    try:
        # Create the job client
        jobs_client = JobsClient()

        # Get the job path
        job_path = jobs_client.job_path(PROJECT_ID, REGION, SPATIAL_JOIN_JOB_NAME)
