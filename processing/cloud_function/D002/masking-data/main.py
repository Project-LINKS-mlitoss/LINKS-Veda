...
MASKING_DATA_JOB_NAME = os.getenv('MASKING_DATA_JOB_NAME')
...
def masking_data(request):
    ...
    try:
        ...
        return send_response(
            {'status': 'ok', 'ticketId': ticket_id, "responseType": "json", 'message': '処理が成功しました。'})
...
def trigger_job(data):
    print("[MASKING DATA] calling trigger job")
    try:
        # Create the job client
        jobs_client = JobsClient()

        # Get the job path
        job_path = jobs_client.job_path(PROJECT_ID, REGION, MASKING_DATA_JOB_NAME)
